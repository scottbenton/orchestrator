...
export function useAcpSession(opts: UseAcpSessionOptions): UseAcpSessionResult {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (!mountedRef.current || !Array.isArray(saved) || saved.length === 0)
          return;
        ...
        const restored = saved.map((m) => {
          ...
          if (m.type === "permission_request" && !m.resolved)
  ...
  useEffect(() => {
    ...
    const { existingSessionId, cwd, acpCommand, acpArgs, workspaceId, tabId } =
      opts;
      ...
      try {
        ...
        if (existingSessionId) {
          try {
            ...
            console.warn(
              "[ACP] failed to resume session, starting fresh:",
              err,
            );
        ...
        if (handle.availableModels.length > 0)
          setAvailableModels(handle.availableModels);
        if (handle.availableModes.length > 0)
          setAvailableModes(handle.availableModes);
      ...
      switch (kind.type) {
        case "message_chunk": {
          setMessages((prev) => {
            ...
            if (last?.type === "assistant" && last.streaming) {
              return [
                ...prev.slice(0, -1),
                { ...last, chunks: [...last.chunks, kind.text] },
              ];
            }
            ...
            return [
              ...prev,
              { type: "assistant", id, chunks: [kind.text], streaming: true },
            ];
          });
          break;
        }
        ...
        case "plan": {
          setMessages((prev) => {
            ...
            return [
              ...closed,
              { type: "plan", id: crypto.randomUUID(), entries: kind.entries },
            ];
          });
          break;
        }
...
function closePendingAssistant(
  messages: ConversationMessage[],
): ConversationMessage[] {
